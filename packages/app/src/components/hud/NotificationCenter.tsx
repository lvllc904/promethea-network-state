        ...
        <>
            ...
            <button 
                ...
                className={`fixed top-4 right-4 z-[9000] p-2 rounded-full backdrop-blur border transition-all ${
                    unreadCount > 0 
                        ? 'bg-cyan-900/40 border-cyan-500 text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.3)] animate-pulse' 
            ...
            >
                <div className="relative">
                    ...
                    {unreadCount > 0 && (
                        <span className="absolute -top-1 -right-1 w-3 h-3 bg-cyan-500 rounded-full text-[8px] flex items-center justify-center text-black font-bold">
                            {unreadCount}
                        </span>
                    )}
                </div>
            ...
            {/* Notification Drawer */}
            {isOpen && (
                <div className={`fixed top-16 right-4 w-80 max-h-[70vh] z-[9000] glass-panel rounded-xl flex flex-col overflow-hidden shadow-2xl border-cyan-500/20 ${reduceAnimations ? '' : 'slide-hud-right'}`}>
                    <div className="p-3 border-b border-white/10 bg-teal-950/40 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] font-black uppercase tracking-widest text-cyan-400">TPNS Notifications</span>
                        ...
                        {notifications.length === 0 ? (
                            ...
                            notifications.map(notif => (
                                <div key={notif.id} className={`p-3 rounded-lg border ${
                                    ...
                                    notif.type === 'update' ? 'bg-cyan-950/30 border-cyan-500/30' :
                                    notif.type === 'warning' ? 'bg-amber-950/30 border-amber-500/30' :
                                    notif.type === 'success' ? 'bg-emerald-950/30 border-emerald-500/30' :
                                    'bg-white/5 border-white/10'
                                }`}>
                                    <div className="flex items-start gap-3">
                                        <div className="mt-0.5">
                                            {notif.type === 'update' && <Info className="w-4 h-4 text-cyan-400" />}
