...
import { useCollection, useFirestore, useSovereignMemo } from '@promethea/sovereign-store';
...
type Props = {
  onAllocate: (data: AllocateRWATasksInput) => Promise<AllocateRWATasksOutput | { error: string }>;
};
  ...
  return (
    <Card className="shadow-lg bg-background/50">
      <CardHeader>
        ...
        <CardDescription>
          Get AI-powered recommendations for matching member skills with tasks to optimize sweat equity deployment.
        </CardDescription>
      </CardHeader>
      <CardContent>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {suggestedMembers.map(member => {
                  const memberAvatar = PlaceHolderImages.find(p => p.id === `user${member.id.slice(0, 1)}`);
                  return (
                    <div key={member.id} className="flex items-center justify-between p-3 rounded-lg border bg-card/80">
                      <div className="flex items-center gap-4">
                        <Avatar className="h-12 w-12 border">
                          {memberAvatar && <Image src={memberAvatar.imageUrl} alt={member.displayName} width={48} height={48} data-ai-hint={memberAvatar.imageHint} />}
                          <AvatarFallback className="text-lg">{member.displayName?.charAt(0)}</AvatarFallback>
                        </Avatar>
