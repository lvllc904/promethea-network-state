  ...
  const cockpitTabs = [
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-6 bg-gray-900 rounded border border-gray-800 border-l-4 border-l-orange-500 relative overflow-hidden">
                 ...
                 <div className="flex justify-between items-center mb-2">
                    ...
                    <Badge className={`${brokerData?.authenticated ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'} uppercase text-[8px] font-black`}>
                    ...
                    <tbody className="divide-y divide-gray-900">
                       {brokerData?.positions && brokerData.positions.length > 0 ? brokerData.positions.map((p: any) => (
                          <tr key={p.symbol} className="group hover:bg-white/5 transition-colors">
                             ...
                             <td className={`py-3 text-[10px] font-mono font-bold text-right ${p.unrealizedPnl >= 0 ? 'text-amber-400' : 'text-red-400'}`}>
      ...
      content: (
        <div className="space-y-6">
          ...
          <div className="p-4 bg-gray-950 rounded border border-gray-900 grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                  { label: 'Inflow', val: `$${intel?.totalInflow || '0.00'}`, color: 'text-amber-400' },
                  { label: 'API Burn', val: `$${intel?.apiBurn || '0.00'}`, color: 'text-orange-400' },
                  { label: 'Sovereign ROI', val: `${intel?.roi || '1.0'}x`, color: 'text-amber-400' },
    ...
    },
    {
      ...
      content: (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2">
          {methods?.map((m) => (
            <div key={m.id} className="p-3 bg-gray-900 border border-gray-800 rounded hover:border-amber-500/50 transition-all group relative overflow-hidden">
              ...
              <div className="mt-4 space-y-2">
                 ...
                 <div className="h-1 bg-black rounded-full overflow-hidden">
                    <div className="h-full bg-amber-500/50" style={{ width: '94%' }}></div>
                 </div>
              </div>
              ...
              <div className="flex justify-between items-end mt-4">
                 <div className="flex flex-col gap-1">
                    ...
                    <span className="text-[8px] text-amber-400 font-mono">ROI: {m.roi || '1.2'}x</span>
                 </div>
                 <select className="bg-black border border-gray-800 text-[8px] font-bold uppercase p-1 rounded cursor-pointer hover:border-amber-500/50 transition-colors">
                    <option>PRO</option>
                    <option>FLASH</option>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
             {marketplaceAssets && marketplaceAssets.length > 0 ? marketplaceAssets.map((asset: any, i: number) => (
                <div key={asset.id || i} className="p-4 bg-gray-900 border border-gray-800 rounded hover:border-amber-500/30 transition-all group relative overflow-hidden">
                   {/* Header Row */}
                   <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                         {asset.constitutionalAlignment && (
                            <span title="Constitutionally Aligned" className="text-[8px] font-black bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded uppercase tracking-widest">⚖ Aligned</span>
                         )}
                         <span className="text-[8px] font-mono font-bold bg-amber-900/30 text-amber-400 px-2 py-0.5 rounded uppercase">
                   ...
                   {asset.visualContext && (
                      <div className="mb-4 relative rounded-lg overflow-hidden border border-gray-800 bg-gray-900 aspect-video group">
                         ...
                         <div className="absolute bottom-2 left-2 flex items-center gap-2">
                            ...
                            <span className="text-[8px] font-mono text-amber-400 font-bold uppercase tracking-widest">Reality Sync: OK</span>
                         </div>
                      </div>
                   )}
                   ...
                   <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                      {asset.humanAbundanceScore !== undefined && (
                         <div className="flex flex-col p-2 bg-black rounded border border-gray-800">
                            ...
                            <span className="text-lg font-black font-mono text-amber-400">{asset.humanAbundanceScore}</span>
                            <div className="mt-1 h-0.5 bg-gray-800 rounded overflow-hidden"><div className="h-full bg-amber-500/50" style={{ width: `${asset.humanAbundanceScore}%` }}></div></div>
                         </div>
                      )}
                      {asset.capitalVelocityScore !== undefined && (
                         <div className="flex flex-col p-2 bg-black rounded border border-gray-800">
                            ...
                            <span className="text-lg font-black font-mono text-amber-400">{asset.capitalVelocityScore}</span>
                            <div className="mt-1 h-0.5 bg-gray-800 rounded overflow-hidden"><div className="h-full bg-amber-500/50" style={{ width: `${asset.capitalVelocityScore}%` }}></div></div>
                         </div>
                      )}
                   ...
                   <div className="flex items-center justify-between border-t border-gray-800 pt-3">
                      <div className="flex items-center gap-4">
                         <div className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-2">
                               ...
                               >
                                  ...
                                  <span className="text-[8px] font-mono text-amber-400">{asset.yesVotes || 0}</span>
                               </button>
                               <button 
                         ...
                         <div className="flex items-center gap-1">
                            ...
                            <span className="text-[8px] font-mono text-amber-400">${(asset.fundingTotal || 0).toLocaleString()}</span>
                         </div>
                      </div>
                      <button 
                        onClick={() => handleFund(asset.id, 100)} // Default micro-funding contribution
                        className="bg-amber-900/30 border border-amber-500/30 hover:bg-amber-600 text-[8px] font-black uppercase tracking-widest px-4 py-2 text-amber-100 hover:text-black rounded transition-all"
                      >
                         Fund &amp; Execute →
                      </button>
                   </div>
           ...
           <div className="p-6 bg-gray-900 rounded border border-gray-800">
              ...
              <div className="space-y-4">
                 ...
                 <button 
                  onClick={handleSweep}
                  disabled={isSweeping}
                  className="w-full mt-4 py-3 bg-amber-600 hover:bg-amber-500 text-black text-xs font-black uppercase rounded tracking-widest transition-all"
                 >
               ...
               <div className="divide-y divide-gray-800">
                  {intel?.transactions?.slice(0, 6).map((tx: any) => (
                    <div key={tx.id} className="py-3 flex justify-between items-center group cursor-pointer hover:bg-black/40 px-2 -mx-2">
                       ...
                       <span className={`text-xs font-mono font-bold ${tx.type === 'in' ? 'text-amber-400' : 'text-gray-500'}`}>
                          {tx.amount}
                       </span>
                    </div>
                  ))}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-6 bg-gray-900 rounded border border-gray-800 border-l-4 border-l-amber-500">
                 ...
                 <div className="mt-6 flex gap-2">
                    <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/20 uppercase text-[8px] font-black">Net Yield: 14.2%</Badge>
                    <Badge className="bg-amber-500/10 text-amber-400 border-amber-500/20 uppercase text-[8px] font-black">Delta Neutral</Badge>
                 </div>
              </div>
                 ...
                 <tbody className="divide-y divide-gray-900">
                    {[
                       ...
                       <tr key={o.name} className="group hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-3">
                             <Button size="sm" variant="outline" className="h-6 text-[8px] uppercase font-black px-2 border-gray-800 group-hover:border-amber-500/50">Details</Button>
                          </td>
                       </tr>
                    ))}
                 </tbody>
    ...
    <div className="h-screen py-6 px-4">
      <SovereignCockpit 
        ...
        stats={[
           ...
           { label: 'UVT Backing', value: '1.20x', color: 'text-amber-400' }
        ]}
        actions={[
