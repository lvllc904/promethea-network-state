    ...
    if (result) {
        return (
            <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="p-6 bg-black/40 border border-emerald-500/20 rounded-xl space-y-4 relative overflow-hidden">
                    <div className="absolute top-0 right-0 bg-emerald-500/10 text-emerald-400 px-3 py-1 text-[10px] font-black tracking-widest uppercase rounded-bl-lg border-b border-l border-emerald-500/20">
                    ...
                    <div className="space-y-6 max-h-[50vh] overflow-y-auto custom-scrollbar pr-4 text-sm text-zinc-300">
                        {result.legalDocs.sections.map((sec: any, idx: number) => (
                            <div key={idx} className="space-y-2">
                                <h3 className="text-cyan-400 font-bold uppercase tracking-wide border-b border-white/5 pb-1">{sec.heading}</h3>
                    ...
                    <div className="pt-4 border-t border-white/10 flex justify-between items-center text-xs font-mono">
                        ...
                        <span className="text-emerald-400">{result.legalDocs.hash}</span>
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-3">
                    <button 
                        ...
                        className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-black font-black uppercase tracking-widest rounded transition-all"
                    >
                        Enter Syndicate →
                    </button>
                </div>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
                <div className="space-y-4 p-6 bg-black/40 border border-white/10 rounded-xl">
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full bg-black/60 border border-white/10 rounded px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500/50"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            className="w-full bg-black/60 border border-white/10 rounded px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500/50 appearance-none"
                        >
                    ...
                    <div>
                        ...
                        <textarea 
                            ...
                            className="w-full h-32 bg-black/60 border border-white/10 rounded px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500/50 resize-none"
                        />
                    </div>
                </div>

                <div className="bg-cyan-950/20 border border-cyan-500/20 rounded-xl p-5 flex items-start gap-4">
                    <BrainCircuit className="w-6 h-6 text-cyan-400 mt-1 flex-shrink-0" />
                    <div>
                        <h4 className="text-sm font-bold text-cyan-400 uppercase tracking-widest">Promethea Legal Synthesis</h4>
                ...
                <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button 
                        ...
                        className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-black font-black uppercase tracking-widest rounded transition-all text-sm flex items-center gap-2 disabled:opacity-50"
                    >
                        {isGenerating ? (
