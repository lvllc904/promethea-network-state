...
// Dynamic import for client-only D3 component
...
const AppContent: React.FC = () => {
    ...
    return (
        <div className="relative w-screen h-screen flex overflow-hidden bg-black text-white">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed left-0 top-0 h-full w-96 bg-gray-950/90 backdrop-blur-3xl border-r border-gray-800/50 z-50 transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
// ... (Keeping exact sidebar code for brevity, but will include full snippet in logic)
                ...
                <div className="absolute top-8 right-8 p-5 bg-gray-950/80 backdrop-blur-2xl rounded-3xl border border-gray-800/50 shadow-3xl flex flex-col gap-4 min-w-[220px]">
                    ...
                    {Object.entries(CATEGORY_COLORS).map(([cat, color]) => (
                        <div key={cat} className="flex items-center gap-3 px-1 group cursor-default">
                            <div className="w-2.5 h-2.5 rounded-full shadow-[0_0_10px_currentColor] transition-transform group-hover:scale-150" style={{ color: color as string, backgroundColor: 'currentColor' }}></div>
