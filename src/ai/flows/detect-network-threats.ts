...
export async function detectNetworkThreats(input: DetectNetworkThreatsInput): Promise<DetectNetworkThreatsOutput> {
  return detectNetworkThreatsFlow(input);
}
...
`, // Ensure schema descriptions are passed to the prompt.
});
...
const detectNetworkThreatsFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (output?.threatDetected) {
      // If a threat is detected, optionally initiate a community vote using the tool.
      try {
        const voteId = await initiateCommunityVote({
          threatDescription: output.threatDescription,
          suggestedAction: output.suggestedAction,
        });
        console.log(`Community vote initiated with ID: ${voteId}`);
      } catch (error) {
        console.error('Failed to initiate community vote:', error);
        // Handle the error appropriately, possibly by logging it or retrying.
      }
    }
    return output!;
  }
